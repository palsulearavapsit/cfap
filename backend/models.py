...
class User(Model):
    ...
    # Relationships
    carbon_entries = db.relationship(
        "CarbonEntry", backref="user", cascade="all, delete-orphan", lazy=True
    )
    recommendations = db.relationship(
        "Recommendation", backref="user", cascade="all, delete-orphan", lazy=True
    )
    challenge_progresses = db.relationship(
        "ChallengeProgress", backref="user", cascade="all, delete-orphan", lazy=True
    )
...
class ChallengeProgress(Model):
    ...
    """Base64 encoded string representing verification proof image (Optional)."""
