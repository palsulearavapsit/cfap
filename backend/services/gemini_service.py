...
def is_gemini_configured() -> bool:
    """Checks whether the Gemini API key is properly configured on the server (Item 52)."""
...
def get_fallback_recommendations(data: dict) -> List[Dict[str, Any]]:
    ...
    electronics_emissions: float = (
        data.get("shopping_electronics", 0) * ELECTRONICS_FACTOR
    )
                    ...
                    normalized.append(
                        {
                            ...
                            "description": str(
                                rec.get(
                                    "description",
                                    "Lower utility waste by turning off unused electronics.",
                                )
                            ),
                            "difficulty": difficulty,
                            "expected_reduction": float(
                                rec.get("expected_reduction", 10.0)
                            ),
                            "estimated_savings": float(
                                rec.get("estimated_savings", 5.0)
                            ),
                        }
                    )
                return normalized
